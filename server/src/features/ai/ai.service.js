...
function ensureDocumentId(documentId) {
  if (!mongoose.Types.ObjectId.isValid(documentId)) {
    throw new AppError("Document not found.", 404);
  }
}

async function findOwnedDocumentOrThrow(userId, documentId) {
...
function buildSummaryInput(document, truncatedDocument) {
  return [
...
function buildChatInput({ document, truncatedDocument, history, message }) {
  return [
    `Document title: ${document.title}`,
    `Original file name: ${document.originalName}`,
    `Document content${truncatedDocument.truncated ? " (truncated)" : ""}:`,
...
async function getRecentChatHistory({ userId, documentId }) {
...
export async function summarizeDocument({ user, documentId, signal }) {
...
export async function streamDocumentChat({ user, documentId, message, signal, onReady, onChunk }) {
  const document = await findOwnedDocumentOrThrow(user.id, documentId);
  const usage = await consumeAiQuota(user);
  const truncatedDocument = truncateText(document.extractedText, config.aiMaxDocumentChars);
...
export async function listChatMessages({ userId, documentId, query }) {
