...
function buildTitle(file, requestedTitle) {
...
async function enforceUploadLimits(user, file) {
...
export async function createDocumentFromUpload({ user, file, title }) {
  if (!file) {
...
export async function updateDocument({ userId, documentId, title }) {
  const document = await findOwnedDocumentOrThrow(userId, documentId);
